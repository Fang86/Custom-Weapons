...
local previousVector = Vec(0,0,0)
...
function draw()
	...
	function activate()
		...
		for i,v in ipairs (explosions) do
			if v[3] == 0 then
				...
				table.insert(activeExplosives, {v[1], v[2], v[3]})
				previousTimer = 0
				previousDelay = 0
			end
		end
    ...
    local hit, dist = Raycast(plyTransform.pos, direction, GetInt("options.clickExplode.ExplosionMaxDistance"))
    if hit and GetBool("options.clickExplode") then
      ...
      if ((UiIsMouseDown() and GetBool("options.clickExplode.Hold")) or UiIsMousePressed()) and (GetString("game.player.tool") == Tools[unrestrictedTool] or unrestrictedTool == 11) then
		...
		if GetBool("options.clickExplode.Uncap") then
			explosionSize = (GetInt("options.clickExplode.ExplosionSizeUncappedPrecision")/10) + GetInt("options.clickExplode.ExplosionSizeUncapped")
		end
		if GetBool("options.clickExplode.NotifDisplay") and not GetBool("options.clickExplode.ClickFire") and not GetBool("options.clickExplode.Detonator") then
		...
		if not GetBool("options.clickExplode.Detonator") then
			...
			local explosiveTimer = GetFloat("options.clickExplode.DetonatorTimer")
			local explosiveDelay = GetFloat("options.clickExplode.DetonationDelay")
			local distanceRestriction = 0.5
			if explosionSize <= 0.4 then
				distanceRestriction = 0.1
			end
			if VecLength(VecSub(hitPos,previousVector)) > distanceRestriction then
				if GetBool("options.clickExplode.DetonatorNotifs") then
					SetString("hud.notification", "Explosion Added: " .. #explosions+1)
				end
				previousVector = hitPos
				if explosiveDelay == -13 then
					explosiveDelay = 0
				elseif explosiveDelay == -12 then
					explosiveDelay = 0.001
				elseif explosiveDelay == -11 then
					explosiveDelay = 0.005
				elseif explosiveDelay == -10 then
					explosiveDelay = 0.01
				elseif explosiveDelay == -9 then
					explosiveDelay = 0.05
				elseif explosiveDelay <= 0 then
					explosiveDelay = 0.9 + explosiveDelay/10
				end
				--SetString("hud.hintinfo", "Beginning " .. previousTimer .. " " .. previousDelay)
				if previousDelay ~= 0 then
					if previousTimer ~= 0 then
						explosiveTimer = explosiveTimer + previousTimer + previousDelay
					else
						explosiveTimer = explosiveTimer + previousDelay
					end
				end
				previousTimer = explosiveTimer
				previousDelay = explosiveDelay
				--SetString("hud.hintinfo", GetInt("clickExplode.PreviousTimer") .. " " .. GetInt("clickExplode.PreviousDelay"))
				--SetString("hud.notification", explosiveTimer .. " " .. explosiveDelay)

				table.insert(explosions, {hitPos, explosionSize, explosiveTimer})
			end
		end
      end
    end
